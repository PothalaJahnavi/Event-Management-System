import React, { useEffect, useState } from "react";
import { useFirebase } from "../context/firebase";
import { useNavigate } from "react-router-dom";

const Event = (data) => {
  const firebase = useFirebase();
  const navigate = useNavigate();
  const user = firebase?.currentUser;
  const islogin = firebase?.loggedIn;
  const role = firebase?.currentRole;
  const [participatedEvents, setParticipatedEvents] = useState();
  const [newEvent, setNewEvent] = useState(true);
  const handleEditEvent = async (id) => {
    if (!islogin) {
      alert("Login to Edit your event");
      navigate("/admin-login");
      return;
    } else {
      navigate(`edit-event/${id}`);
    }
  };
  const handleParticipate = async () => {
    if(!newEvent){
      alert("You have already participated")
      return;
    }
    if (!islogin) {
      alert("Login to Participate event");
      navigate("/login-user");
      return;
    }
    try {
      const res = await firebase.participteInEvent(user?.uid, data.index);
      if (res) {
        alert("Hurray!! You participated in this event");
        navigate("/user-dashboard");
      } else {
        alert("There is some error!Please try again");
      }
    } catch (err) {
      alert("There is some error");
    }
  };

  useEffect(() => {
    const fetchData = async () => {
      const res = await firebase.UserEvents();
      setParticipatedEvents(res);
      console.log(res,data.index)
      if (participatedEvents?.some((item) => item.id === data?.index)) {
        setNewEvent(false);
      }
    };
    fetchData();
    console.log(newEvent)
  }, []);

  return (
    <div class="card" style={{width:'350px'}}>
      <img src="data:image/jpeg;base64," />
      <div class="card-body">
        <h5 class="card-title text-success">{data?.event?.name}</h5>
        <p class="card-text">{data?.event?.desc}</p>
        <p>{new Date(data?.event?.time).toLocaleString()}</p>
        {role === "admin" && user?.uid === data?.event?.adminId && (
          <a
            href="#"
            class="btn btn-primary"
            onClick={() => handleEditEvent(data.index)}
          >
            Edit Event
          </a>
        )}
        {role == "user" &&
          new Date(data?.event?.time) > new Date() &&
          newEvent && (
            <a href="#" class="btn btn-primary" onClick={handleParticipate}>
              Participate
            </a>
          )}
        {role == "user" &&
          new Date(data?.event?.time) > new Date() &&
          !newEvent && (
            <a href="#" class="btn btn-secondary">
              Registered
            </a>
          )}
      </div>
    </div>
  );
};

export default Event;
